...
server.tool("play-random-music", "Play a random audio file from a music directory using Rhythmbox", {
    directory: zod_1.z.string().optional().describe("Music directory (default: ~/Musique)"),
    recursive: zod_1.z.boolean().optional().default(true).describe("Search subdirectories recursively"),
    exts: zod_1.z.array(zod_1.z.string()).optional().describe("List of audio extensions to include, e.g., ['mp3','flac']"),
    player: zod_1.z.string().optional().default("rhythmbox").describe("Player command to run (default: 'rhythmbox')")
}, {
    ...
    try {
        const resolveTilde = (p) => p.startsWith('~') ? node_path_1.default.join(userHomeDir, p.slice(1)) : p;
        const baseCandidates = [];
        if (directory && directory.trim()) {
            baseCandidates.push(resolveTilde(directory.trim()));
        }
        else {
            baseCandidates.push(node_path_1.default.join(userHomeDir, 'Musique'), node_path_1.default.join(userHomeDir, 'Music'), node_path_1.default.join(userHomeDir, 'music'));
        }
        const exists = async (p) => !!await promises_1.default.stat(p).then(() => true).catch(() => false);
        let baseDir = '';
        ...
        const walk = async (dir) => {
            ...
            for (const ent of entries) {
                ...
                if (ent.isDirectory()) {
                    ...
                    const ext = node_path_1.default.extname(ent.name).slice(1).toLowerCase();
        ...
        const withCtx = (cmd) => `DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u)/bus" ${cmd}`;
        const attempts = [
        ...
        const pickName = node_path_1.default.basename(pick);
        const errors = [];
...
server.tool("add-calendar-event", "Create a calendar event (.ics) and open it for import in GNOME Calendar (Ubuntu)", {
    ...
    start: zod_1.z.string().describe("Start datetime in ISO 8601, e.g. 2025-08-10T14:00:00"),
    end: zod_1.z.string().optional().describe("End datetime in ISO 8601. If not provided, durationMinutes is used"),
    durationMinutes: zod_1.z.number().int().positive().optional().default(60).describe("Duration in minutes if end not provided"),
    description: zod_1.z.string().optional().default("").describe("Event description"),
    location: zod_1.z.string().optional().default("").describe("Event location"),
    allDay: zod_1.z.boolean().optional().default(false).describe("All-day event"),
    reminderMinutes: zod_1.z.number().int().min(0).max(1440).optional().default(30).describe("Reminder before event in minutes (0 to disable)"),
}, {
    ...
    try {
        ...
        const eventsDir = node_path_1.default.join(userHomeDir, 'Calendar', 'events');
        await promises_1.default.mkdir(eventsDir, { recursive: true });
        const fileNameSafe = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 64) || 'event';
        const filePath = node_path_1.default.join(eventsDir, `${fileNameSafe}-${now.getTime()}.ics`);
        await promises_1.default.writeFile(filePath, icsContent, 'utf8');
...
server.tool("list-calendar-events", "List saved .ics events with basic details", {
    directory: zod_1.z.string().optional().describe("Directory containing .ics files (default: ~/Calendar/events)"),
    limit: zod_1.z.number().int().min(1).max(500).optional().default(100).describe("Max number of events to list"),
    sort: zod_1.z.enum(["asc", "desc"]).optional().default("desc").describe("Sort by file mtime (newest first by default)")
}, {
    ...
    try {
        const baseDir = directory
            ? (directory.startsWith('~') ? node_path_1.default.join(userHomeDir, directory.slice(1)) : directory)
            : node_path_1.default.join(userHomeDir, 'Calendar', 'events');
        await promises_1.default.mkdir(baseDir, { recursive: true });
        const files = (await promises_1.default.readdir(baseDir)).filter(f => f.toLowerCase().endsWith('.ics'));
        const entries = await Promise.all(files.map(async (f) => {
            const full = node_path_1.default.join(baseDir, f);
        ...
        const details = await Promise.all(pick.map(async (e) => {
            const content = await promises_1.default.readFile(e.path, 'utf8');
...
server.tool("delete-calendar-event", "Delete a saved .ics event by UID or filename", {
    uid: zod_1.z.string().optional().describe("Event UID to delete (from the .ics contents)"),
    file: zod_1.z.string().optional().describe("Filename or absolute path to the .ics to delete"),
    directory: zod_1.z.string().optional().describe("Events directory if deleting by filename (default: ~/Calendar/events)")
}, {
    ...
    try {
        ...
        let targetPath = null;
        if (file) {
            targetPath = file.startsWith('~') ? node_path_1.default.join(userHomeDir, file.slice(1)) : (node_path_1.default.isAbsolute(file) ? file : node_path_1.default.join(userHomeDir, 'Calendar', 'events', file));
        }
        else if (uid) {
            const baseDir = directory
                ? (directory.startsWith('~') ? node_path_1.default.join(userHomeDir, directory.slice(1)) : directory)
            ...
            for (const f of files) {
                const p = node_path_1.default.join(baseDir, f);
                const content = await promises_1.default.readFile(p, 'utf8');
...
server.tool("update-calendar-event", "Update fields of an existing .ics event identified by UID", {
    uid: zod_1.z.string().describe("UID of the event to update"),
    directory: zod_1.z.string().optional().describe("Events directory (default: ~/Calendar/events)"),
    title: zod_1.z.string().optional().describe("New title"),
    description: zod_1.z.string().optional().describe("New description"),
    location: zod_1.z.string().optional().describe("New location"),
    start: zod_1.z.string().optional().describe("New start datetime ISO 8601"),
    end: zod_1.z.string().optional().describe("New end datetime ISO 8601"),
    durationMinutes: zod_1.z.number().int().positive().optional().describe("If end not provided, use duration from start"),
    allDay: zod_1.z.boolean().optional().describe("Set as all-day event"),
    reminderMinutes: zod_1.z.number().int().min(0).max(1440).optional().describe("Reminder minutes (0 disables)")
}, {
    ...
    try {
        const baseDir = args.directory
            ? (args.directory.startsWith('~') ? node_path_1.default.join(userHomeDir, args.directory.slice(1)) : args.directory)
            : node_path_1.default.join(userHomeDir, 'Calendar', 'events');
        await promises_1.default.mkdir(baseDir, { recursive: true });
        const files = (await promises_1.default.readdir(baseDir)).filter(f => f.toLowerCase().endsWith('.ics'));
        let targetPath = null;
        ...
        const pad = (n) => String(n).padStart(2, '0');
        const toUTCStamp = (d) => (d.getUTCFullYear().toString() +
            pad(d.getUTCMonth() + 1) +
            pad(d.getUTCDate()) + 'T' +
            pad(d.getUTCHours()) +
            pad(d.getUTCMinutes()) +
            pad(d.getUTCSeconds()) + 'Z');
        const toDateOnly = (d) => (d.getUTCFullYear().toString() +
            pad(d.getUTCMonth() + 1) +
            pad(d.getUTCDate()));
        const now = new Date();
        const dtstamp = toUTCStamp(now);
        ...
        lines.push('END:VEVENT');
        lines.push('END:VCALENDAR');
        await promises_1.default.writeFile(targetPath, lines.join('\n'), 'utf8');
...
server.tool("export-calendar-events", "Export .ics events from a directory as JSON or CSV", {
    directory: zod_1.z.string().optional().describe("Events directory (default: ~/Calendar/events)"),
    format: zod_1.z.enum(["json", "csv"]).optional().default("json").describe("Export format")
}, {
